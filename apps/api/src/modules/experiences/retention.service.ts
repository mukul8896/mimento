...
export class RetentionService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  keptUntil(lastActivityAt: Date): Date | null {
    return this.env.RETENTION_DAYS > 0
      ? new Date(lastActivityAt.getTime() + this.env.RETENTION_DAYS * DAY_MS)
      : null;
  }

  /** The creator is back: they, and everything they have made, count as active. */
  async touchOwner(ownerId: string, lastSeenAt: Date, now = new Date()): Promise<void> {
    ...
    await this.prisma.experience.updateMany({
      where: { ownerId, status: { not: 'DELETED' }, lastActivityAt: { lt: stale } },
      data: { lastActivityAt: now, retentionWarnedAt: null },
    });
  }

  /** One surprise was used (its manage link, or a recipient opening it). */
    ...
    for (;;) {
      ...
      for (const { id } of stale) {
        await this.prisma.$transaction(async (tx) => {
          // Re-check inside the transaction: someone may have opened it a moment ago.
          const locked = await tx.$queryRaw<{ id: string }[]>`
            SELECT "id" FROM "Experience"
            WHERE "id" = ${id}::uuid AND "status" <> 'DELETED' AND "lastActivityAt" < ${cutoff}
            FOR UPDATE`;
